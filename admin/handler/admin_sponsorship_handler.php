...
switch ($action) {
    ...
    case 'delete':
        ...
        if ($id) {
            ...
            $stmt->bind_param("i", $id);
            if ($stmt->execute()) {
                if ($image_path_to_delete && file_exists($_SERVER['DOCUMENT_ROOT'] . $image_path_to_delete)) {
                    unlink($_SERVER['DOCUMENT_ROOT'] . $image_path_to_delete);
                }
    ...
    case 'edit':
        ...
        if (isset($_FILES['sponsorshipImage']) && $_FILES['sponsorshipImage']['error'] == UPLOAD_ERR_OK) {
            ...
            $image_name = basename($_FILES["sponsorshipImage"]["name"]);
            $unique_name = uniqid() . '_' . $image_name;
            $new_image_path = $target_dir . $unique_name;
