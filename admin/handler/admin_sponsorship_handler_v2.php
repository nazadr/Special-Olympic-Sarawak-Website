...
switch ($action) {
    case 'add':
        ...
        // For backward compatibility, also check for old type field
        if (empty($sponsor_name) && !empty($_POST['type'])) {
            $sponsor_name = 'Sponsor';
        }
        
        // Handle legacy type field for backward compatibility
        $type = $_POST['type'] ?? '';
        if (!empty($type) && empty($chapter_input)) {
        ...
        if ($chapter_id === null) {
            ...
            $type = getChapterName($conn, $chapter_id);
            if (strpos($type, 'Chapter') === false) {
                // Get city name and append " Chapter"
        ...
        // Check if new columns exist, if not use legacy insert
        $columns_check = $conn->query("SHOW COLUMNS FROM sponsorships LIKE 'sponsor_name'");
        $has_new_columns = $columns_check->num_rows > 0;

        if ($has_new_columns) {
            $stmt = $conn->prepare("INSERT INTO sponsorships (sponsor_name, type, chapter_id, sponsor_tier, image_path, website_url, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, 1)");
            $stmt->bind_param("ssisssi", $sponsor_name, $type, $chapter_id, $sponsor_tier, $image_path, $website_url, $display_order);
        } else {
    ...
    case 'fetch_grouped':
        // Fetch sponsors grouped by chapter
        $columns_check = $conn->query("SHOW COLUMNS FROM sponsorships LIKE 'sponsor_name'");
        $has_new_columns = $columns_check->num_rows > 0;
    ...
    case 'fetch_by_chapter':
        ...
        if ($chapter_id === '0' || $chapter_id === 'state') {
            ...
            if ($has_new_columns) {
                ...
                // Legacy: map chapter_id to type string
    ...
    case 'edit':
        ...
        if ($id) {
            ...
            if ($has_new_columns) {
                $stmt = $conn->prepare("UPDATE sponsorships SET sponsor_name = ?, type = ?, chapter_id = ?, sponsor_tier = ?, image_path = ?, website_url = ?, display_order = ? WHERE id = ?");
                $stmt->bind_param("ssisssii", $sponsor_name, $type, $chapter_id, $sponsor_tier, $image_path, $website_url, $display_order, $id);
            } else {
